...
> 生成日期: 2026-03-09
> 最后更新: 2026-03-14（第六次更新 — **修复5项未完成任务**: 孤儿CRR根治、审核链端到端集成修复、前端审核链进度集成、getPendingStepsForRole方法补齐、路由注释统一）
...
>
> **🟢 后端实施状态**: 已完成（4张表 + 4个模型 + ApprovalChainService + 路由 + Seed数据 + 超时服务 + Web前端页面 + 端到端集成修复）
...
> **2026-03-14 校准**: 本节已与真实数据库 `roles` 表完全对齐。所有 role_id 和 role_level 以 Section 1.1 为权威来源。

```
位置 1: 提交位   (merchant_staff role_level=20, role_id=10)
位置 2: 初审     (business_manager role_level=60, role_id=7 — 当前审核链路由准入线)
位置 3~8: 预留扩展
位置 9: 终审     (admin role_level=100, role_id=2, is_final 强制为 true)
```

**实际可用审核角色（数据来源: roles + user_roles 表，2026-03-14 查询）:**

| 角色 | role_id | role_level | 当前用户数 | 能否做审核人 |
|------|---------|-----------|-----------|------------|
| admin | 2 | 100 | 5 | 可以，最高权限，可审核任何步骤 |
| regional_manager | 6 | 80 | 0 | 架构上可以，但当前无用户分配 |
| business_manager | 7 | 60 | 2 | 可以，审核链路由准入线（`requireRoleLevel(60)`） |
| merchant_manager | 11 | 40 | 1 | 低于准入线(60)，不可直接进入审核路由 |
| ops | 9 | 30 | 1 | 只读角色，不参与审核 |
| merchant_staff | 10 | 20 | 0 | 提交位，不做审核人 |

> **说明**: `content_auditor` 角色在数据库中不存在（从未创建）。当前审核人体系为 admin(5人) + business_manager(2人)。如未来日均审核 > 100 单，可创建 content_auditor 角色（建议 role_level=55）并调整准入线。

### 运营配置实例（基于真实消费金额 ¥20~¥150）

**默认消费链（全部金额）:** 提交 → admin终审(role_id=2) — 与当前单级审核一致

**未来大额消费链 (>200元):** 提交 → business_manager初审(role_id=7) → admin终审(role_id=2)

**商家积分默认链:** 提交 → admin终审(role_id=2)

> 当前消费金额范围窄(¥20~¥150)，初期只配置默认链（单级admin终审）。等业务发展后通过管理后台配置多级链。
...
- `consumption_large`: 2级
  - 节点1: business_manager(**role_id=7**, role_level=60)初审，timeout_hours=12, timeout_action='escalate'
...
| **审核权限过窄** | 当前仅 admin(level=100) 可审核（路由注释明确写了"审核权限只开放给 admin，不开放 ops/区域经理"），未利用 business_manager(role_id=7, level=60) 等中间角色 | P1（**已修复**: 审核链准入线设为 business_manager role_level>=60） |
| **可用审核人有限** | 数据库中不存在 content_auditor 角色（从未创建）。审核链 role 分配初期只能用 admin(5人, role_id=2) 和 business_manager(2人, role_id=7) | P2 |
...
| **审核推送通知** | business_manager(role_id=7, level=60) 和 admin(role_id=2, level=100) 角色需通过 Socket.IO + 微信模板消息接收待审核通知 | P1 |
| **审核操作页面** | business_manager(role_id=7, level>=60) 及以上角色需在小程序内审批（如果开放给非admin角色） | P2 |
...
> **2026-03-14 校准**: 数据库中**不存在** `content_auditor` 角色（从未创建）。business_manager（role_id=7, role_level=60）有 2 个用户，是当前除 admin 外唯一可做审核人的角色。

**核心问题**: 审核链的初审节点，是否需要创建 content_auditor 专岗角色，还是用现有 business_manager(role_id=7) 兼任？
...
|------|---------|---------|-------------|----------------|------------|---------|
| **选项A: content_auditor 专岗** | content_auditor（需创建角色，建议 role_id 待定, level=55） | 否（需创建角色+分配用户） | 取决于配多少人 | N/A 已是专岗 | 极低（职责清晰） | 日均审核 > 200 单 |
| **选项B: business_manager 兼任** ✅ | business_manager(role_id=7, level=60) | **是（2人就绪）** | ~500单/天（2人） | **零代码，改模板 role_id** | 低 | 日均审核 < 200 单 |

#### 关键洞察: 为什么两个方案的长期维护成本差别不大

审核链的设计精妙之处在于: **谁来审不是代码决定的，是 `approval_chain_nodes.assignee_role_id` 这个数据库字段决定的**。

- 现在填 `business_manager(role_id=7)` → business_manager 兼审
- 未来创建 content_auditor 角色后改填其 role_id → 专岗审核
...
#### **最终决策: 选项B — business_manager(role_id=7, level=60) 兼任初审**
...
## 十三、2026-03-14 实施更新记录

> 更新日期: 2026-03-14
> 更新内容: 修复5项未完成任务（3项后端P1+2项前端P1+2项后端P2）

### 13.1 已修复问题清单

| # | 问题 | 优先级 | 修复方案 | 状态 |
|---|------|--------|---------|------|
| 1 | `getPendingStepsForRole()` 方法缺失 | P2 | 在 `ApprovalChainService` 中添加按角色查询待审核步骤方法 | ✅ 已修复 |
| 2 | 路由注释 `content_auditor(55)` 与实际 `requireRoleLevel(60)` 不一致 | P2 | 统一所有 JSDoc 注释为 `business_manager(role_level >= 60)` | ✅ 已修复 |
| 3 | 孤儿 CRR 仍在产生（数据一致性 P1） | P1 | 见 13.2 根因分析 | ✅ 已修复 |
| 4 | 审核链端到端集成未触发（0实例） | P1 | 见 13.2 根因分析（与问题3同源） | ✅ 已修复 |
| 5 | finance-management.html 未集成审核链信息 | P1 | 消费列表新增"审核链"列，详情弹窗新增步骤时间线 | ✅ 已修复 |
| 6 | 审核详情时间线未实现 | P1 | 新增 `chainTimelineModal` 弹窗，展示完整审核链步骤进度 | ✅ 已修复 |

### 13.2 孤儿CRR + 审核链未触发 — 根因分析与修复

**根因分析：**

```
消费提交流程（修复前）:
  路由层 → CoreService.merchantSubmitConsumption()
           ├── 创建 ConsumptionRecord (事务内)
           └── 直接创建 ContentReviewRecord (事务内)  ← 问题所在
                ❌ 绕过了 ContentAuditEngine.submitForAudit()
                ❌ 审核链匹配代码在 ContentAuditEngine 中，从未被调用

ContentAuditEngine.submitForAudit() 内部流程:
  创建 CRR → 匹配模板 → 创建链实例
  ✅ 此方法包含审核链集成代码
  ❌ 但从未被 consumption 提交流程调用
```

**修复方案：**

1. **`CoreService.merchantSubmitConsumption()`**: 移除直接创建 CRR 的代码，改为调用 `ContentAuditEngine.submitForAudit()` 统一创建审核记录，后者自动匹配审核链模板并创建实例
2. **`ContentAuditEngine.submitForAudit()`**: 加强事务约束，使用 `assertAndGetTransaction()` 强制要求外部事务传入，与项目事务边界规范一致
3. **孤儿数据清理**: 迁移 `20260314205016` 硬删除 6 条孤儿 CRR（IDs: 5290, 5291, 5318, 5319, 5333, 5334）
...
消费提交流程（修复后）:
  路由层 → CoreService.merchantSubmitConsumption()
           ├── 创建 ConsumptionRecord (事务内)
           └── ContentAuditEngine.submitForAudit() (同一事务)
                ├── 创建 ContentReviewRecord
                ├── ApprovalChainService.matchTemplate() ← 匹配模板
                └── ApprovalChainService.createChainInstance() ← 创建审核链实例
```
...
| 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|
| GET | `/api/v4/console/approval-chain/instances/by-auditable` | 按业务记录查询审核链实例 | business_manager(60+) |

**参数**：`auditable_type` (string) + `auditable_id` (number)
**用途**：finance-management.html 消费列表和详情弹窗查询审核链进度

### 13.4 新增服务方法

| 服务 | 方法 | 说明 |
|------|------|------|
| `ApprovalChainService` | `getPendingStepsForRole(roleId, queryOptions)` | 按角色ID查询待审核步骤（支持分页和业务类型筛选） |

### 13.5 前端变更

**finance-management.html 消费记录模块：**

1. **列表新增"审核链"列**: 显示"第X步/共N步"标签，点击可查看时间线
2. **详情弹窗集成审核链进度**: 展示各步骤审核人、意见、时间
3. **新增 `chainTimelineModal`**: 完整审核链步骤时间线弹窗（包含时间线节点、审核意见、审核人信息）
4. **自动预加载**: 列表加载时异步预加载 pending 记录的审核链状态

**approval-chain.js API 模块：**

- 新增 `INSTANCE_BY_AUDITABLE` 端点常量
- 新增 `getInstanceByAuditable(auditableType, auditableId)` 方法

### 13.6 数据库迁移

| 迁移文件 | 说明 |
|----------|------|
| `20260314205016-cleanup-orphan-crr-and-fix-chain-integration.js` | 硬删除 6 条 pending 孤儿 CRR |
| `20260314210453-cleanup-historical-orphan-crr.js` | 硬删除 454 条历史孤儿 CRR（cancelled:445 + rejected:5 + approved:4） |
| `20260314223852-add-approval-chain-audit-operation-types.js` | 新增审核链审计操作类型常量 |
| `20260314224150-cleanup-orphan-crr-4353.js` | 硬删除孤儿 CRR id=4353（对应消费记录已软删除） |

### 13.7 未变更项说明

| # | 项目 | 说明 |
|---|------|------|
| 1 | `content_auditor` 角色 | 维持不创建的决策。当前使用 business_manager(level=60) 作准入线，触发条件未满足 |
| 2 | Section 5 角色数据 | ✅ 已修正（2026-03-14），role_id 和 role_level 已与真实数据库完全对齐 |
| 3 | `matchTemplate()` 逻辑 | 代码层面正确（空对象 `{}` 返回 true），问题在于该方法从未被调用（已修复） |
...
| # | 问题 | 修复方式 | 优先级 |
|---|------|----------|--------|
| 1 | ApprovalChainTimeoutService 3 处 AdminNotification 字段名错误 | `type` → `notification_type`, `source` → `source_type`, 值改为合法 ENUM | P0 |
| 2 | 审核链创建/推进/完成时不通知审核人 | createChainInstance/advanceToNextStep/processStep 末尾写入 AdminNotification | P0 |
| 3 | 前端 approveConsumption/confirmReject 不处理 redirect 响应 | 增加 redirect 判断，显示提示并打开审核链时间线 | P0 |
| 4 | 审核链操作无审计日志 | 新增 APPROVAL_CHAIN_CONFIG/AUDIT 操作类型，路由层 AuditLogService.logOperation() | P1 |
| 5 | getPendingStepsForRole 无路由暴露 | 新增 `GET /role-pending` 路由 (admin only) | P1 |
| 6 | Socket.IO 实时推送未实现 | 路由层审核通过/拒绝后推送 broadcastNotificationToAllAdmins | P1 |
| 7 | 残留孤儿 CRR id=4353 | 迁移硬删除（对应消费记录 id=2022 已软删除） | P2 |
| 8 | 待处理中心未集成审核链 | PendingCenterService 新增 approval_chain 分类，pending-center.html 新增卡片 | P2 |

### 14.2 确认已存在（文档声称缺失但实际已实现）

| # | 项目 | 实际状态 |
|---|------|----------|
| 1 | `getPendingStepsForRole(roleId)` 方法 | ✅ 已存在于 ApprovalChainService.js (line 433) |
| 2 | `GET /instances/by-auditable` 路由 | ✅ 已存在于 routes/v4/console/approval-chain.js (line 205) |
| 3 | 前端 `getInstanceByAuditable()` API | ✅ 已存在于 admin/src/api/approval-chain.js (line 85) |
| 4 | finance-management.html 审核链列、详情、时间线 | ✅ 已存在（table column line 303, detail modal line 1433, timeline modal line 2014） |
| 5 | `INSTANCE_BY_AUDITABLE` 端点常量 | ✅ 已存在于 admin/src/api/approval-chain.js (line 23) |

### 14.3 新增 API 端点

| 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|
| GET | `/api/v4/console/approval-chain/role-pending` | 按角色ID查询待审核步骤 | admin(100+) |

**参数**：`role_id` (number, 必填) + `auditable_type` (string, 可选) + `page` + `page_size`

### 14.4 通知机制

审核链全生命周期通知（AdminNotification + Socket.IO）：

| 事件 | 通知对象 | notification_type | source_type |
|------|----------|-------------------|-------------|
| 链创建 | 第一步审核人（角色池所有用户） | task | approval_chain |
| 步骤推进 | 下一步审核人 | task | approval_chain |
| 终审通过 | 提交人 | system | approval_chain |
| 终审拒绝 | 提交人 | alert | approval_chain |
| 超时升级 | 被升级到的审核人 | task | approval_chain_timeout |
| 终审超时提醒 | 当前审核人 | alert | approval_chain_timeout |
| 普通超时提醒 | 当前审核人 | reminder | approval_chain_timeout |

### 14.5 端到端验证状态

审核链模板（3个，全部激活）：
- `consumption_default`: 消费审核默认链（1 审核节点，admin 终审）
- `consumption_large`: 消费审核大额链（2 审核节点，business_manager 初审 → admin 终审，金额 >= 200 触发）
- `merchant_points_default`: 商家积分审核默认链

审核链实例：0 条（自集成以来未有新消费提交，需要通过实际消费提交验证完整链路）

**验证建议**：
1. 提交金额 < 200 消费 → 走 `consumption_default`（单级 admin 终审）
2. 提交金额 >= 200 消费 → 走 `consumption_large`（business_manager 初审 → admin 终审）
3. 验证通过/拒绝流程 → 确认 ContentAuditEngine.approve()/reject() → ConsumptionAuditCallback 回调链
4. 验证通知 → 确认 AdminNotification 和 Socket.IO 推送

### 14.6 文档 vs 代码对齐状态

Section 13 中 6 项声称"已修复"的状态更新：

| # | 文档声称 | 实际状态（2026-03-15） |
|---|----------|----------------------|
| 1 | getPendingStepsForRole 已修复 | ✅ 代码 + 路由均已完成 |
| 2 | 消费提交绕过 ContentAuditEngine | ✅ 代码已修改，但 0 实例（未经真实数据验证） |
| 3 | 孤儿 CRR 根治 | ✅ 完成（含 id=4353 最后一条） |
| 4 | 审核链端到端集成修复 | ✅ 代码完成 + 通知 + 审计日志，待真实数据验证 |
| 5 | finance-management.html 集成审核链 | ✅ 完全实现（列 + 详情 + 时间线） |
| 6 | chainTimelineModal | ✅ 完全实现 |
