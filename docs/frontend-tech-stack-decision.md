...
| 现有组件 / 场景 | 处理结果 |
|----------------|----------|
| `custom-tab-bar` | 已接入 TDesign `t-tab-bar` |
| `components/loading-spinner` | 已删除，通用场景改用 `t-loading` |
| `components/empty-state` | 已删除，通用场景改用 `t-empty` |
| `components/category-cascade` | 已删除，分类选择改用 `t-cascader`，数据拉取移到页面/父组件层 |
| `prize-detail-modal` | 已改为 TDesign `t-popup` 外壳，保留内容区 |
| 通用空态/加载态页面 | 已开始统一到 `t-loading` / `t-empty`，广告列表、广告详情、反馈列表、反馈详情、反馈历史弹窗、工单、审核列表、发现页、我的挂单、交易市场已收口 |
| `styles/variables.scss` | 已删除，`styles/td-overrides.scss` 已全局接管 |

#### 待继续收口的

| 现有组件 / 场景 | 说明 |
|----------------|------|
| `components/pagination` | 旧全局分页已删除；exchange 模块已改为滚动触底加载，当前仓库的 TDesign 版本不含独立 pagination 组件 |
| 交易/审核类页面中的局部分页 UI | 仍需逐页决定是保留自定义页码还是改为滚动加载 |
| 少量业务型 loading/empty | 骨架屏、图表、支付、上传等强业务场景暂保留 |
