...
- 前端近期又对齐了工作台（diy-design）的一批能力到 diy-lite：**撤销/重做 30 步历史栈、手串珠子多选批量删除、分享链接还原设计、加购前登录鉴权、售罄禁购（stock===0）、素材类型 Tab（饰品/配饰/吊坠）**。其中前四项为纯前端；**售罄依赖后端 `stock` 字段**（见第 6 节口径）；**配饰/吊坠 Tab 当前显示空态**，素材需后端提供（见第 10 节）。
...
- **本需求不是从零建接口**，而是：① 复用现有接口；② 请后端在 `DiyBead` 上**补充 6 个渲染/展示字段**（material_type/meaning/weight/energy/pairing/five_elements）；③ 补充异形珠几何元数据；④ 明确几项业务规则由后端下发。
...
| `bead-data.ts` 的 `five_elements`(暂空) | `DiyBead.five_elements`（五行雷达图用）      |
...
10. 购物车/报价/支付的下单闭环接口口径（见 10.2）；镶嵌模板 `slot_definitions` 是否可下发（见 10.3）。
...
| 分享好友 | 分享路径带 `?beads=珠子id清单&wrist=手围&wear=戴法`，好友打开按本地数据还原 | 分享路径带 `?workId=xxx`，好友打开用 `GET /diy/works/:id` 还原对方的设计 |
...
diy-lite 当前「加入购物车」为**前端演示计数**（已加登录鉴权拦截，未登录先弹登录）。正式版下单闭环需要后端明确：

| 环节       | 前端现状             | 需要后端提供                                                    |
| ---------- | -------------------- | --------------------------------------------------------------- |
| 加入购物车 | 本地计数 + toast演示 | 购物车接口（增/删/查），或确认走「保存作品→结算」模式无需购物车 |
| 提交设计   | 无                   | `POST /api/v4/diy/works` 保存作品（同第 9 节）                  |
| 报价/费用  | 前端按演示单价累加   | 服务端权威报价（总价/明细），前端只展示不计算                   |
| 支付       | 无                   | 支付/结算接口与流程约定（对齐工作台 diy-result 的支付面板体系） |

### 10.3 镶嵌/槽位模式（暂未在 diy-lite 实现，需后端数据才能启动）

工作台（diy-design）支持「镶嵌模式」（模板 `layout.shape === 'slots'`，槽位有 `allowed_diameters/allowed_shapes/allowed_group_codes` 约束）。该模式**完全依赖后端模板的 `slot_definitions` 业务数据**（槽位位置/尺寸/约束均为商品设计数据，前端无权威依据、不能编造）。diy-lite 暂不实现镶嵌模式；若业务需要在 diy-lite 提供，请后端先确认镶嵌模板数据可下发，前端再开发渲染与交互。
