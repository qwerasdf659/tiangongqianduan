/**
 * 价格走势图组Canvas 2D 轻量折线 *
 * 零依赖：使用微信小程Canvas 2D API 绘制折线 * 不引echarts00KB+），适合小程序包体积限制
 *
 * 后端API:
 *   GET /api/v4/marketplace/price/trend   价格走势数据 *   GET /api/v4/marketplace/price/summary 价格摘要统计
 *
 * 使用方式:
...
const CHART_COLORS = {
  /** 价格折线 品牌橙色 */
  line: '#5B7A5E',
  /** 折线下方填充（实色，$gradient-blue-start 一致） */
  fillSolid: '#5B7A5E',
  /** 网格*/
...
Component({
  properties: {
    /** 资产代码（如 'red_core_shard'），templateId 二选一 */
  ...
  data: {
    ...
    /** 价格摘要统计 */
    summary: null as any,
    /** 当前选中的图表标签页: 'price' 价格走势 / 'volume' 成交*/
  ...
  methods: {
    ...
    async _fetchData() {
      ...
      try {
        /**
         * 90天周期使getPriceHistory（长周期分析数据         * 其他周期使用 getPriceTrend（短周期发现数据         */
        const trendPromise =
