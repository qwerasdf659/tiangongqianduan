 ...
 *
 * 包含交易市场 Tab 的全部业务方法：
 *   - 挂单列表加载（服务端筛排序/分页 *   - 搜索/筛排序（全部参数传递给后端 QueryService *   - 筛选维度从 facets API 动态获 *   - 购买弹窗
 *
 * 筛选机制（C+++方案，对齐独立交易市场页market.ts）：
 *   - listing_kind / sort / category_id / rarity_code / asset_group_code
 *     / asset_code / min_price / max_price / with_counts 全部通过 API 查询参数传 *   - 筛选维度从 GET /api/v4/marketplace/listings/facets 动态获 *
 * 后端API:
 *   GET /api/v4/marketplace/listings（QueryService 嵌套响应结构 *   GET /api/v4/marketplace/listings/facets（筛选维度字典表 *
 * @file components/exchange/exchange-market/handlers/market-behavior.ts
 * @version 7.0.0
 * @since 2026-03-15
 */
...
module.exports = Behavior({
  methods: {
     ...
     *
     * 数据
     *   筛选状构建API参数 GET /api/v4/marketplace/listings?listing_kind=xxx&sort=xxx
     *   后端 QueryService WHERE/ORDER BY 返回筛选结     *   enrichProductDisplayFields setData渲染
     */
    async loadProducts(append: boolean = false) {
      ...
      try {
        ...
        /**
         * 构建后端API查询参数（对齐独立市场页market.ts 的完整筛选参数集         * 后端 GET /api/v4/marketplace/listings 支持:
        ...
        if (response && response.success && response.data) {
          ...
          /**
           * 适配后端 QueryService 嵌套响应，使ImageHelper 统一图片降级           * fungible_asset 类型 本地材料图标（按 asset_code 映射           * item 类型 后端 primary_media.public_url 分类图标 占位           */
          const processedProducts = items.map((item: any, idx: number) => {
            if (!item.market_listing_id) {
              marketLog.warn(
                `products[${idx}] 缺少 market_listing_id，后端响应字`,
                Object.keys(item)
              )
            }
            ...
            /* 品质等级视觉配置（后item_info.instance_attributes*/
            ...
            /* 限量编号（后item_info.serial_number + item_info.edition_total*/
          ...
          const interleavedProducts = this._interleaveExchangeFeedAds(enrichedProducts)

          const mergedProducts = append
            ? [...(this.data.filteredProducts || []), ...interleavedProducts]
            : interleavedProducts
    ...
    /**
     * 加载筛选维度数据（首次展开高级筛选面板时触发     * 后端API: GET /api/v4/marketplace/listings/facets
     */
    async loadFacets() {
      ...
      try {
        ...
        if (facetsResponse && facetsResponse.success && facetsResponse.data) {
          ...
          marketLog.info('筛选维度加载成')
        }
      } catch (facetsError: any) {
        marketLog.warn('加载筛选维度失', facetsError.message)
      }
    },

    /** 切换高级筛选面板（首次展开时加facets*/
    ...
    /**
     * 打开担保码输入弹     * listing_kind = 'item' 的实物交易需要担保码
     */
    ...
    /**
     * 提交担保码确认收     * POST /api/v4/marketplace/trade-orders/:trade_order_id/confirm-delivery
     *
     * ⚠️ 需要后Phase 4 EscrowCodeService 实施完成后才可调     */
    async onSubmitEscrowCode(_e?: any) {
      ...
      try {
        ...
        if (result && result.success) {
          marketLog.info('担保码确认成', escrowTradeOrderId)
          ...
          throw new Error((result && result.message) || '担保码验证失')
        }
      } catch (error: any) {
        marketLog.error('担保码确认失', error)
    ...
    /**
     * 加载兑换商城 feed 信息流广     *
     * 对应广告 exchange_list_feed（ID:15，日5星石，CPM 3星石     * 广告穿插由前端控制，后端只负责下发广告内     */
    async _loadExchangeFeedAds() {
      try {
        ...
        marketLog.info('兑换商城Feed广告加载成功:', feedItems.length, '')
      } catch (feedError) {
    ...
    /**
     * feed 广告穿插到商品列表中
     * 穿插规则: 每隔 EXCHANGE_FEED_AD_INTERVAL 条商品插入一条广     */
        ...
        switch (adData.link_type) {
          ...
          case 'miniprogram':
            wx.navigateToMiniProgram({
              ...
              fail: (err: any) => marketLog.error('广告跳转小程序失', err)
            })
            break
          ...
          default:
            marketLog.warn('未知的广告跳转类', adData.link_type)
        }
      }
    },

    /**
     * Feed 广告事件上报 campaign_category 分流
     */
