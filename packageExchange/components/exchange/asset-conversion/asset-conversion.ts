 ...
 *   3. 执行转换（POST /api/v4/assets/conversion/convert *   4. 展示每日限额使用情况
 *   5. 通过 triggerEvent 通知 Page 壳刷新积分余 *
 * 后端API（统一资产转换规则026-04-05 合并 exchange_rates + material_conversion_rules
 ...
 * 注意：后DECIMAL/BIGINT 字段返回字符串，前端使用时需 Number() 转换
 *
 * @file components/exchange/asset-conversion/asset-conversion.ts
 * @version 7.1.0
...
Component({
  ...
  data: {
    ...
    /** conversion_type 分组后的 Tab 列表（后端虚拟字 compose/decompose/exchange*/
    ...
    /** 错误提示 */
    errorMessage: '',
    /** 兑换成功弹窗确认按钮配置（Skyline 不支WXML 内联对象字面量） */
    resultConfirmBtn: { content: '知道了' }
  },

  observers: {
    /** refreshToken 变化时重新加载转换规则列*/
  ...
  methods: {
    /**
     * 加载所有可用转换规     * 后端: GET /api/v4/assets/conversion/rules
     * 注意: 后端 DECIMAL/BIGINT 字段返回字符串，需 Number() 转换后使     */
    async _loadConversionRules() {
      ...
      try {
        ...
        /* conversion_type 构建 Tab 分组（后端虚拟字 compose/decompose/exchange*/
    ...
    onFillMaxAmount() {
      ...
      /* 根据预览返回的余额确定最大值，否则properties 积分余额做粗*/
      const preview = this.data.previewResult
    ...
    /** 切换转换类型 Tab（合分解/兑换*/
    onSwitchTab(e: any) {
      const tabType = e.detail?.value || e.currentTarget?.dataset?.type
