 ...
 *
 * @file components/exchange/exchange-shelf/sub/bid-panel/bid-panel.ts
 * @version 5.2.0
 * @since 2026-02-21
 */
...
Component({
  ...
  methods: {
    ...
    async loadBidProducts() {
      ...
      try {
        ...
        if (response && response.success && response.data) {
          ...
          bidLog.info(`获取${bidProducts.length} 个竞价商品`)
    ...
    async onConfirmBid() {
      ...
      wx.showModal({
        title: '确认竞价',
        content: `您将${userBidAmount} ${bidImageHelper.getAssetDisplayName(selectedBidProduct.price_asset_code || '')} 出价，资产将被冻结直到被超越或竞价结束。`,
        confirmText: '确认出价',
        cancelText: '再想',
    ...
    _formatBidCountdown(endTime: string | number | undefined): string {
      ...
      if (hours > 24) {
        return `${Math.floor(hours / 24)}{hours % 24}时`
      }
      if (hours > 0) {
        return `${hours}{minutes}分`
      }
      if (minutes > 0) {
