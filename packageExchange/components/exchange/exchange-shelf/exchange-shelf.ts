 ...
 *
 * 职责 *   1. 接收 Page 壳下传的 properties（积分、空间、刷新令牌）
 *   2. 管理空间切换（幸臻选）、臻选解锁状 *   3. 通过 CSS 变量注入样式配置（style="{{shelfThemeStyle}}" *   4. 通过 selectComponent 驱动子组件刷 *   5. 通过 triggerEvent 通知 Page 壳（兑换成功、积分变动、认证错误）
 *
 * 决策D10: 使用 Behavior 替代 spread handler
 * 决策D12: 子组件自行管理筛分页状态，shelf 仅保~29 字段
 * 决策D13: 使用 hidden 替代 wx:if，组件始终存 *
...
Component({
  ...
  methods: {
     ...
     *   - 商城筛 product-filter 配置（权威来源，使用 categories.category_code     *   - 降级: 如果 product-filter API 失败，回退exchange-page shop_filters
     */
    async _initShelf() {
      try {
        ...
        if (productFilterResult && productFilterResult.success && productFilterResult.data) {
          ...
          shelfLog.info('使用 product-filter API 筛选配')
        } else {
        ...
        shelfLog.info('货架初始化完')
      } catch (error) {
        shelfLog.error('货架初始化失', error)
    ...
    /**
     * 商品点击事件 跳转详情     * 后端字段: exchange_item_id（exchange_items 表主键）
     */
    onProductTap(e: any) {
      ...
      wx.navigateTo({
        ...
        fail: (navError: any) => {
          shelfLog.error('跳转详情页失败，降级为弹窗确', navError)
          this.setData({
     ...
     * 请求 { exchange_item_id, quantity, sku_id }（全SKU 模式sku_id 由后端校验库存与价）
     * 请求 Idempotency-Key（幂等键，HTTP Header，非 body     */
    async onConfirmShopExchange() {
      ...
      /** 货架一键兑换：仅「单默认 SKU」可直接sku_id；多规格必须进详情（与详情页全量 SKU 逻辑一致） */
    ...
    /** 视图模式切换（组件内联按triggerEvent 通知 Page 壳） */
    ...
    onBidComplete(_e: any) {
      shelfLog.info('竞价完成，通知 Page 壳刷新积')
      this.triggerEvent('pointsupdate')
    },

    /** 设置错误状*/
    setErrorState(errorMessage: string, _errorDetail: string) {
      shelfLog.info('设置错误状', errorMessage)
      this.setData({ loading: false })
