 ...
 *   3. 统一处理13种标准玩法的抽奖API调用
 *   4. 管理组件内部运行时状态（架构决策1.5：不依赖lotteryStore）
 *   5. 根据 size 属性适配展示模式（full/medium/small/mini）
 *
 * 数据来源：调用真实后端API
 *
...
/** 引入API工具、认证助手 — 统一从 utils/index 导入 */
...
const DEFAULT_DISPLAY = {
  mode: 'grid_3x3',
  grid_cols: 3,
  effect_theme: DEFAULT_THEME_NAME,
...
Component({
  ...
  data: {
    ...
    winAnimation: 'simple',
    /**
     * 活动背景图URL（当前以后端文档定义的 display.background_image_url 为唯一权威字段）
     */
  ...
  methods: {
    ...
    async initActivity(campaignCode: string) {
      ...
      try {
        ...
        /**
         * 活动级主题：活动配置了 effect_theme 就使用活动主题，否则固定使用 default 单主题
         */
        const activeEffectTheme = configDisplay.effect_theme || DEFAULT_THEME_NAME
