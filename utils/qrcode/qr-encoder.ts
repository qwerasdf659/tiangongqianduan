...
class Polynomial {
  ...
  mod(other: Polynomial): Polynomial {
    ...
    for (let i = 0; i < this.getLength(); i++) result[i] = this.get(i)
    for (let i = 0; i < other.getLength(); i++) result[i] ^= gexp(glog(other.get(i)) + ratio)
...
class BitBuffer {
  ...
  putBit(bit: boolean): void {
    ...
    if (bit) this.buffer[idx] |= 128 >>> this.length % 8
    this.length++
  }
}

// === RSBlock ===
interface RSBlockEntry { totalCount: number; dataCount: number }
...
const FULL_RS_TABLE = [...RS_BLOCK_TABLE, ...RS_BLOCK_TABLE_2, ...RS_BLOCK_TABLE_3, ...RS_BLOCK_TABLE_4]

function getRsBlockTable(typeNumber: number, errorCorrectLevel: ErrorCorrectLevel): number[] | undefined {
  const ecIndex = [1, 0, 3, 2].indexOf(errorCorrectLevel)
  if (ecIndex < 0) return undefined
...
function getLengthInBits(mode: number, type: number): number {
  if (type >= 1 && type < 10) {
    if (mode === 1) return 10; if (mode === 2) return 9
    if (mode === 4 || mode === 8) return 8
  } else if (type < 27) {
    if (mode === 1) return 12; if (mode === 2) return 11
    if (mode === 4) return 16; if (mode === 8) return 10
  } else if (type < 41) {
    if (mode === 1) return 14; if (mode === 2) return 13
    if (mode === 4) return 16; if (mode === 8) return 12
  }
...
export class QRCode {
  ...
  static createData(typeNumber: number, ecLevel: ErrorCorrectLevel, dataList: QR8BitByte[]): number[] {
