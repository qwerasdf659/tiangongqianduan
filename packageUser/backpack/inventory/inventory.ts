 ...
 * @file packageUser/backpack/inventory/inventory.ts
 * @version 5.2.0
 * @since 2026-02-15
 */
...
const { getQualityGradeStyle, formatEdition } = ImageHelper
...
Page({
  ...
  onLoad(_options: any) {
    ...
    this.backpackBindings = createStoreBindings(this, {
      store: backpackStore,
   ...
   * 3. 基于后端 allowed_actions 数组计算 WXML 操作标志（can_use / can_generate_code）
   * 4. 计算分类统计数量
   * 5. 独立加载背包统计数据
   * 6. 应用当前筛选排序条件
   *
  ...
  async loadInventoryData(refresh: boolean = false) {
    ...
    try {
      ...
      if (success && data) {
         ...
         *
         * 业务规则（后端权威，前端不硬编码）:
         *   product/voucher → ["redeem"]         实物/券需到店核销，不支持线上"使用"
         *   prize           → ["redeem"]         奖品到店核销
         *   service         → ["use"]            线上权益直接激活
         *   ⚠️ C2C 用户间交易已整体下线，后端不再下发 "sell" 动作；
         *      前端无任何"挂卖/转让/出售"入口（兑换得到的实例 is_tradable=0）
         */
  ...
  onUnload() {
    ...
    if (this.backpackBindings) {
      this.backpackBindings.destroyStoreBindings()
    }
    clearTimeout(this.searchTimer)
    if (this._qrTimer) {
      clearInterval(this._qrTimer)
  ...
  onShareAppMessage() {
    return {
      title: '我的仓库',
      path: '/packageUser/backpack/inventory/inventory'
    }
  }
})
