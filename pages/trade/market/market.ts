 ...
 * 布局计算委托给共享工具 utils/waterfall.ts，消除重复实现
 *
...
// 🔴 统一工具函数导入（外部页面统一从 utils/index 导入）
...
Page({
  ...
  async loadProducts(page: number = 1, append: boolean = false) {
    if (this.data.loading) {
      marketLog.info('⚠️ 正在加载中，跳过重复请求')
      return
    }

    marketLog.info(`📦 开始加载商品数据 - 第${page}页`)
    this.setData({ loading: true })

    try {
      const result = await API.getMarketProducts(page, this.data.pageSize)
      ...
      if (success && data && data.products) {
        ...
        marketLog.info(`✅ 商品数据加载完成 - 共${layoutResult.layoutProducts.length}个商品`)
      } else {
  ...
  loadProductImage(productId: string) {
    marketLog.info(`🖼️ 开始加载商品图片: ${productId}`)
  ...
  onProductClick(e: WechatMiniprogram.TouchEvent) {
    ...
    marketLog.info('🎯 商品点击:', product.name)
  ...
  /** 🗑️ 页面卸载清理 */
  onUnload() {
    marketLog.info('🗑️ 瀑布流布局页面卸载，清理资源')
