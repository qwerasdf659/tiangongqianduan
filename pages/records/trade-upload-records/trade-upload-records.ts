/**
 * 📊 trade-upload-records.ts - 积分活动记录页面（交消费合并 MobX响应式状态 *
 * @description
 * 将交易记录和消费记录合并到一个页面，使用Tab切换行 * Tab0 = 交易记录（资产流水），Tab1 = 消费记录（商家消费积分）
 *
 * @version 5.2.0
...
// 从Utils中解构认证助手和格式化函const { checkAuth, formatPoints } = Utils
// 从Wechat中解构Toast函数
const { showToast } = Wechat

/**
 * 安全转换时间字段为字符串
 * 防止后端返回Sequelize Date对象等非字符串类型导[object Object]
 *
 * @param value - 需要转换的时间隔 * @returns 时间字符号 */
...
Page({
  ...
  data: {
    ...
    transactionRecords: [] as API.AssetTransaction[],
    filteredRecords: [] as API.AssetTransaction[],
    // 筛选条件
    ...
    consumptionRecords: [] as API.ConsumptionRecord[],
...
onTabChange(e: WechatMiniprogram.BaseEvent) {
  ...
  /**
   * 💰 加载积分余额汇   * API: GET /api/v4/assets/balance?asset_code=POINTS
   *
   ...
   *
   * 后端实际返回字段（对typings/api.d.ts AssetTransaction）：

   *   asset_transaction_id - 交易流水ID（BIGINT PK
   *   asset_code           - 资产代码（POINTS / DIAMOND / red_shard
   *   delta_amount         - 变动金额（正获得/earn，负消费/consume
   *   balance_before       - 变动前余
   *   balance_after        - 变动后余
   *   business_type        - 业务类型枚举（lottery_consume / lottery_reward / exchange_debit 等）
   *   description          - 交易描述（约44%有值，可为null   *   title                - 交易标题（约33%有值，可为null   *   created_at           - 创建时间
   *
   * 前端处理：将后端 snake_case 字段映射WXML 模板所需的显示字符   */
  ...
  try {
    ...
    if (success && data) {
      ...
      if (transactions.length > 0) {
        log.info('🔍 [诊断] 第一条原始交易记', {
          keys: Object.keys(transactions[0]),
          sample: transactions[0]
        })
      }

      // 处理交易记录 保留后端原始字段，仅新增前端计算的显示字符
...
/**
 * 🔴 应用筛选条   * 使用处理后的字段名（category/title/description/txn_id   */
applyFilters() {
  ...
  if (this.data.currentTypeFilter !== 'all') {
    if (this.data.currentTypeFilter === 'income') {
      ...
      // 按后business_type 筛选（lottery_consume / exchange_debit 等）
...
onCopyTxnId(e: WechatMiniprogram.BaseEvent) {
  ...
  // ============================================================================
  // 🧾 消费记录相关方法
  // 后端API: GET /api/v4/shop/consumption/me（支持分+ status筛选）
  // ============================================================================

  /**
   * 🧾 加载消费记录
   *
   * 后端返回字段（基consumption_records 表，对齐v2.0文档
   ...
   *   - status: ENUM pending/approved/rejected/expired态，含expired
   *   - final_status: ENUM pending_review/approved/rejected（双重状态）
   *   - merchant_notes: TEXT 商家备注
   *   - admin_notes: TEXT 管理员备   *   - store_id: INT FK 门店ID
  ...
  try {
    ...
    if (success && data) {
      ...
      // 🔴 处理消费记录 - 安全转换时间字段（防[object Object]
...
/**
 * 🔴 格式化审核状态   * @param status - 状态   */
formatReviewStatus(status: string) {
  /* 后端 consumption_records 表有4? pending/approved/rejected/expired */
 ...
 * 🧾 查看消费记录详情
 *
 * @description 显示消费记录的详细信息弹   * 字段来源：后端API GET /api/v4/shop/consumption/me 返回的记   * 使用status字段（与WXML模板和后端API一致）
 */
