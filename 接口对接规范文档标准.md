...
- **更新时间**：2025年01月11日 - 基于权限简化后的实际运行代码的完整更新
- **使用模型**：Claude Sonnet 4
- **项目类型**：餐厅积分抽奖系统（restaurant-points-backend）
- **产品对接**：100%符合产品功能结构描述文档，确保前后端数据库对接工作顺利进行

### 1.2 🔴 权限系统重大简化（v2.2.0 - 2025年01月11日）

> **⚠️ 重要变更**：权限系统已从三级权限简化为二级权限，所有接口和数据结构都已相应调整
...
### 1.3 🎯 完善说明 - 2025年01月11日权限简化更新
本次更新基于权限系统简化的深度理解，包括：
- ✅ **后端技术规范文档**（权限简化版）- Node.js微服务架构标准
- ✅ **数据库设计规范文档**（权限简化版）- MySQL核心表结构简化
- ✅ **产品功能结构文档**（需要更新）- 业务功能权限调整
- ✅ **项目代码完整分析** - 6个核心路由模块+3个核心服务+8个数据模型
- 🔴 **2025年01月11日权限简化修复**：基于实际运行代码，确保接口规范100%符合简化后的权限实现
- 🔴 **实际验证完成**：
  - 权限中间件 - 简化为requireAdmin单一权限检查（middleware/auth.js）
  - 用户模型 - 简化权限字段，保留is_admin作为管理员权限控制（models/User.js）
- API路由 - 商家管理功能完全保留，仅调整为requireAdmin权限控制（routes/merchant.js）
  - 认证系统 - 统一登录方式，权限通过is_admin识别（routes/auth.js）
...
### 5.3 管理员查看所有用户上传记录接口（需要后端实现）

> **注意**：此接口需要后端程序员实现，前端暂时无法使用
...
```javascript
// WebSocket连接地址
const WS_URL = 'wss://rqchrlqndora.sealosbja.site/ws';
...
BASE_URL="https://rqchrlqndora.sealosbja.site"
